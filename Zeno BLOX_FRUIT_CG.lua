...
function Notify(Des, Time, title)
	Fluent:Notify({
		Title= title or "Zeno Hub Notify",
		Content = Des,
		Duration = Time or 3
	})
end
...
_G['Zeno Hub Table'] = {
	...
	TypeOs = (table.find({Enum.Platform.Windows}, game:GetService('UserInputService'):GetPlatform()) ~= Zeno and 'Pc') or 'Mb',
...
_G['Zeno Hub Table'].SizeUi = (not _G['Zeno Hub Table'].isWindows and UDim2.fromOffset(600,300)) or UDim2.fromOffset(560,600)
if not _G['Zeno Hub Table'].isWindows then
...
UICorner.Parent = ImageButton
	...
	local frame = Frame
	local dragToggle = Zeno
	local dragSpeed = 0.25
	local dragStart = Zeno
	local startPos = Zeno
